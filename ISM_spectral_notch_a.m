function [Q_a_T,am,thetam] = ISM_spectral_notch_a(q_0,r_0,q,a,b,time,Const,Config)
...
%--------------------------%
drawn_transit =0; % Variable for drawing the transition poses
Const.q = q;
...
for it_tubes = Config.nb_tubes :-1:  2
    ...
    Const.it_troncon = 1;
    for it_x = 1:N_nodes
        [~,F_1_e,F_bar(:,it_x)] = F_ext(X_grid(it_x),QX_k{it_tubes}(:,it_x),rX{it_tubes}(:,it_x),0,Const,Config);
    end

    F_1 = [Const.C{it_tubes}; 0; 0; Const.T{it_tubes}; 0; 0];
    CI = F_1;

    CL_ind=[1,N_nodes+1,2*N_nodes+1,3*N_nodes+1,4*N_nodes+1,5*N_nodes+1];

    f_A = @(it) ad_(Xi_k{it_tubes}(:,it))';
    f_B = @(it) -h_k{it_tubes}(it)*F_bar(:,it);
    
    Lambda_X_k{it_tubes} = integral_spectral(f_A,f_B,CI,DX,N_nodes,CL_ind);
...
Const.it_troncon = 1;
for it_x = 1:N_nodes
    [~,F_1_e,F_bar(:,it_x)] = F_ext(X_grid(it_x),QX_k{1}(:,it_x),rX{1}(:,it_x),0,Const,Config);
end
F_1_p = F_1_e;
...
CL_ind=[N_nodes,2*N_nodes,3*N_nodes,4*N_nodes,5*N_nodes,6*N_nodes];

f_A = @(it) ad_(Xi(:,it))';
f_B = @(it) -F_bar(:,it);

Lambda_X_k{1} = integral_spectral(f_A,f_B,CI,DX,N_nodes,CL_ind);
...
for it_tubes = 1 : Config.nb_tubes     
    for it_x = 1:N_nodes
        ...
        delta_h_k_xi_1 = -[0, sin(theta)*Config.D{it_tubes}(X_k{it_tubes}(it_x)),(Config.D{1}(X_grid(it_x))+cos(theta)*Config.D{it_tubes}(X_k{it_tubes}(it_x))),0,0,0]';
         
        coef_dual_epsilon{it_tubes}(:,:,it_x) = Ad_g_(Rk_1,rk_1)' - delta_h_k_xi_1*(1/hk)*Xi_k{it_tubes}(:,it_x)';   

        [Phi,Phi_prime] = Base_Phi_prime(Config.L-X_grid(it_x),time,Const,Config);;
        nb_modes_theta = Const.dim_base_q_theta/(Config.nb_tubes-1);
        mu_k = -[1 0 0 0 0 Config.D{it_tubes}(X_k{it_tubes}(it_x))]';
        
        coef_dual_theta_prime = Phi_prime*mu_k'*Lambda_X_k{it_tubes}(:,it_x);
        theta_0_k = Config.theta_0{it_tubes}(X_k{it_tubes}(it_x));
        delta_h_delta_theta_k = Config.D{it_tubes}(X_grid(it_x)) * (Xi(3,it_x)*sin(theta_0_k) - Xi(2,it_x)*cos(theta_0_k));
        delta_nu_k_delta_theta_k = Config.D_prime{1}(X_grid(it_x))*[0;0;0;0;-sin(theta_0_k);cos(theta_0_k)]; 
...
for it_tubes = 2 : Config.nb_tubes 
    ...
    for i=1:Const.dim_base_q_e
        CL_ind=[CL_ind,(i-1)*N_nodes+1];
    end

    f_A = @(it) zeros(Const.dim_base_q_e,Const.dim_base_q_e);
    f_B = @(it) [Phi_e(:,:,it),zeros(Const.dim_base_q_e,2),Phi_e(:,1,it)*Config.D{1}(X_grid(it))]*(-Lambda_X_k{1}(:,it) + Total_Lambda_X_k(:,it));

    Qa_k_X = integral_spectral(f_A,f_B,CI,DX,N_nodes,CL_ind);
...
Qa_t_X = integral_spectral(f_A,f_B,CI,DX,N_nodes,CL_ind);
